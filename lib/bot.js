...
var Bot = module.exports = function Bot(slackKey, tasksKey, webhookBaseUrl, webhookServer) {
  ...
  slackClient.on('message', function(message) {
    ...
    if (message.upload) {
      ...
      logger.debug(channel, plainTextMessage(message));
      ...
      if (user.state == 'idle') {
        ...
        logger.debug('tasks.createTask', { body: brief, urls: user.files, webhookUrl: webhookBaseUrl + externalId(message), externalId: externalId(message), emailNotifications: 'disabled' });
        tasksClient.createTask({ body: brief, urls: user.files, webhookUrl: webhookBaseUrl + externalId(message), externalId: externalId(message), emailNotifications: 'disabled' }, function(err, task) {
          if (err) {
            logger.error(err);
  ...
  var notify = bot.notify = function(channel, payload) {
    logger.debug('Webhook receieved: channel:%s payload:%s', channel, payload);
    if (payload.task_id) {
      tasksClient.getTask(payload.task_id, function(err, task) {
        if (err) {
